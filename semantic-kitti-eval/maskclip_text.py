...
@dataclass
class ProgramArgs:
    checkpoint_path: str = "../maskclip-edit/pretrain/ViT16_clip_backbone.pth"
    ...
    semantic_preds_dir: str = 'data/SemanticSeg/SemanticKITTI/maskclip_text/'
...
def inference_segmentor_km(model, img, return_feat=True):
...
def main():
    ...
    for frame in tqdm(range(len(os.listdir(kitti_path+'velodyne/')))):
        ...
        _, img_feat = inference_segmentor_km(model, img)
        ...
        #     prompt = prompt.cuda()
        #     text_feat = clip_text_encoder.encode_text(prompt)  # 1, 512
        ...
        # # Image sim stuff
        # img_sim = cosine_similarity2(img_feat.unsqueeze(0).cpu(), text_feats_all.unsqueeze(1).unsqueeze(1).cpu())
        # pred_img = img_sim.argmax(0)
        # pallete = get_new_pallete(10)
