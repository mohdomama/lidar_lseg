...
def create_map():
    ...
    for idx in tqdm(range(797)):
        ...
        # TODO: Load from saved openseg features
        # img_feat = get_img_feat(img, net)
        embfile = './data/lego_loam_map/feat_openseg_640_640/' +keyframe_id+'.pt'
        img_feat = torch.load(embfile)
        

        # Features in PCD Space
        pcd_feat = np.zeros((pcd.shape[0], 768), dtype=np.float32)
        img_feat_np = img_feat.detach().cpu().numpy()[0]
        img_feat_np = np.transpose(img_feat_np, (1,2,0))

        # Not needed here since feat are of same size. Also this might be buggy. 
